...
for x, t in AppleXT: # リンゴのxとTをループ
    if abs(t - Status[1]) >= abs(x - Status[0]): # リンゴが取れる(残り時間が移動可能距離より多い)時
        x_diff = abs(x - Status[0]) # x座標の変化量
        for i in range(x_diff): # カゴの移動(r:right,l:left)
            Result += 'r,' if x > Status[0] else 'l,'
            if (i + 1) % 4 == 0: # x座標が4増加するごとにy座標を1増加
                Status[2] += 1
        for i in range(abs(t - Status[1]) - x_diff): # 余った時間は待機(s:stop)
            Result += 's,'
        Status[0] = x # x座標の更新
        Status[1] = t # t座標の更新
# --- algorithm end ---
