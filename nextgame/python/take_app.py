...
print("AppleXT after adjustment:", AppleXT)
...
print("OpponentXT after adjustment:", OpponentXT)
...
print("Opponent's path:", path_opponent)
...
for idx in path_opponent:
    opponent_apples.add((OpponentXT[idx][0], OpponentXT[idx][1]))

print("Opponent apples:", opponent_apples)

for i in range(n):
    for j in graph[i]:
        if (AppleXT[j][0], AppleXT[j][1]) in opponent_apples:
            if dp[i] + 1 > dp[j]:
                dp[j] = dp[i] + 1
                next_node[j] = i
        else:
